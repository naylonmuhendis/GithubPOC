...
class AdvRecyclerView : RecyclerView {
...
//        addItemDecoration(itemDecor)
    ...
    private fun getOrientation(attrs: AttributeSet?): Int {
        ...
        val orientation = b.getInt(0, VERTICAL)
        b.recycle()
        return orientation
    }
    ...
    private fun decorateView(
        ...
        val typedArray = context.theme.obtainStyledAttributes(
            attrs,
            R.styleable.AdvRecyclerView, 0, 0
        )

        // Divider and the mode
        if (typedArray.hasValue(R.styleable.AdvRecyclerView_rvDividerDrawable)) {
            val dividerDrawable =
                typedArray.getDrawable(R.styleable.AdvRecyclerView_rvDividerDrawable)
            val dividerMode = typedArray.getInt(
                R.styleable.AdvRecyclerView_rvDividerMode,
            ...
            val isExtraSpaceActive = ExtraSpaceMode.EXTRA_SPACE_ON == typedArray.getInt(
                R.styleable.AdvRecyclerView_rvExtraSpace, ExtraSpaceMode.EXTRA_SPACE_ON
            )
        ...
        if (isInEditMode) {
            ...
            val layout = typedArray.getInt(
                R.styleable.AdvRecyclerView_rvPreviewLayout,
                PREVIEW_LAYOUT_AS_LIST
            )
            val layoutManager = when (layout) {
                PREVIEW_LAYOUT_AS_GRID -> GridLayoutManager(context, 2, orientation, false)
